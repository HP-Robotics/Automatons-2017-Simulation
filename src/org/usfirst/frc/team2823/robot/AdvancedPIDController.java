...
public class AdvancedPIDController implements PIDInterface, LiveWindowSendable {
  ...
  /*private double safeArm(double output){
		...
		//if(robot.armEncoderget() < OUTPUT_CLAMP_HOLD_THRESHOLD && output < OUTPUT_CLAMP_HOLD){
  ...
  protected void calculate() {
    ...
    if (enabled) {
      ...
      synchronized (this) {
        input = pidInput.pidGet() * m_invertMultiplier * -27.0;
  ...
  public synchronized double getError() {
    // return m_error;
    return getSetpoint() - (m_pidInput.pidGet() * m_invertMultiplier * -27.0);
  }
